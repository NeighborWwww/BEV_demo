...
1. **镜头的校准 & 画面拼接**
    ...
    -  对于画面的拼接，本项目的做法是找到一个比较合适的位置（Sample1视频的第325帧，车身是相对车道平行，且侧后方有井盖作为车辆参照物）进行两个摄像头拼接位置的校准，测试车道在侧视和后视中的具体比例来验证。这也是我为什么需要开发util.py这个模块来辅助进行各个摄像头ROI的建立，在有限的时间和有限的资源内，这个方法能有效的使画面最大限度的贴合。
    -  对于画面的拼接校准，也是需要地下有明确的道路辅助线，而且多个摄像头之间要留交叉余量在画面边际做羽化叠加处理。这样在开门的过程中，画面不会有太过明显的变化。
    -  可以看到result.mp4在最后阶段也会有一个问题就是太阳直射的问题导致每个角度的光线风格不一致。这个需要对在硬件上对于每一条影像通道都进行滤波，来尽可能减少这个问题带来的影响。同样，这个问题依然是本项目没有时间解决的一个问题。
2. **盲区的产生**
...
3. **时间差问题**
    - 其实你会发现，有的时候过减速带的时候，不同镜头之间的震动不是同步的，这个是因为时间戳没有完全对齐。对于这个问题，在代码中已经保留了offset的能力，有时间的友友可以去对一下时间戳看着更像那么回事。对于生产级别的项目，时间戳是需要更加严格的通过参数传入的。车端的部署会直接使用摄像头的视频信号。如果我们在这里设计多线程和异步读取，也会减少一些性能上的问题。
...
- 如果这个玩意不是用Python写的，改写一下其实真的还能快不少嘞
...
```
BEV/
├── data/              # 视频文件目录（太大了没法传，请根据数据来源自行下载，修改命名方式）
│   ├── sample1_front.mp4
│   ├── sample1_back.mp4
│   ├── sample1_left.mp4
│   ├── sample1_right.mp4
